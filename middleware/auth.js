...
const ensureIdOrAdminAuth = (req, res, next) => {
  ...
  if (req.session.user.user_id === parseInt(req.params.id) || req.session.user.username === 'blearned92') {
    return next();
  } else {
...
const ensureAdminAuthentication = (req, res, next) => {
    if (req.session.authenticated && req.session.user.username === 'blearned92') {
      return next();
    } else {
      res.status(403).json({ message: "You're not authorized to for this action" });
    }
